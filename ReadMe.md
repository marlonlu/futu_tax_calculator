...
### 移动加权平均算法原理
- **买入时**: 对齐富途成本计算公式，买入成本 = 买入价格 × 买入数量 + 手续费
- **买入时**: 更新持仓数量和总成本，重新计算平均成本价
- **卖出时**: 按当前平均成本价计算盈亏，卖出收入 = 卖出价格 × 卖出数量 - 手续费
- **公式**: 新平均成本 = (原持仓总成本 + 新买入总成本) ÷ (原持仓数量 + 新买入数量)

### 算法示例（含手续费）
```
买入100股@$10，手续费$1 → 总成本$1001，平均成本$10.01
买入200股@$15，手续费$2 → 总成本$3002，平均成本$13.34 ((1001+3001)÷300)
卖出150股@$20，手续费$1.5 → 盈利$998.5 ((20×150-1.5)-(13.34×150))，剩余150股@$13.34
```
